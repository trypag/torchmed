...
def main():
    ...
    for file in os.listdir(args.data):
        ...
        if os.path.isdir(filepath) and not file.startswith('__'):
            ...
            df = read_and_aggregate(metric_file, df)
    ...
    if df is not None:
        ...
        for metric_name in col_names:
            ...
            plot_boxplot(ax1, 111, df[metric_name],
                         'boxplot of the ' + metric_name + ' for each label',
                         'labels',
                         metric_name,
                         None, None)

            outfile = os.path.join(args.output, 'boxplot_labels_' + metric_name + '.png')
...
def plot_boxplot(ax, d, df, title, x_axis, y_axis,
    ...
    ax.set_xticklabels(range(0, len(samples)), rotation=35)
